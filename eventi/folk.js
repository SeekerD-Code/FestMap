...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Leggiamo la categoria direttamente dall'input nascosto della pagina HTML!
    const categoriaCorrente = document.getElementById('filter-tipo')?.value || 'folk';

    const eventiCategoriaTotali = rawEventi ? rawEventi
        .map(e => ({
            ...e,
        ...
        .filter(e => {
            const cat = (e.categoria || '').toLowerCase().trim();
            const tipo = (e.tipo || '').toLowerCase().trim();
            return cat === categoriaCorrente || tipo === categoriaCorrente;
        }) : [];

    // Popola i Mesi nel Select usando ESCLUSIVAMENTE gli eventi di questa categoria
    const selectMese = document.getElementById('filter-mese');
    if (selectMese && eventiCategoriaTotali) {
        if (typeof popolaMesiSelect === 'function') {
            popolaMesiSelect(eventiCategoriaTotali);
        } else {
            const mesiDisponibili = [...new Set(eventiCategoriaTotali.map(e => e.data_inizio_grezza ? e.data_inizio_grezza.substring(0, 7) : null))].filter(Boolean).sort();
    ...
    function eseguiFiltroERender() {
        // Chiamiamo il filtro standard passandogli però SOLO la nicchia di eventi di questa categoria
        const filtrati = filtraEventi(eventiCategoriaTotali);
        renderizzaEventi(filtrati);
    }

    // Primo avvio
    if (eventiCategoriaTotali && eventiCategoriaTotali.length > 0) {
        renderizzaEventi(eventiCategoriaTotali);
    }
